...
interface BHJetPoint{t:number;angle:number;size:number;bright:number;hue:number;dist?:number}

const STAR_COUNT=600,PARTICLE_COUNT=2000,JET_COUNT=40
...
function makeParticle():BHParticle{return{angle:ra(0,Math.PI*2),radius:ra(0.3,0.9),size:ra(0.3,2),bright:ra(0.2,0.8),speed:ra(0.002,0.006),life:ra(0.5,1)}}
...
export const preset:PresetModule={
  ...
  createRenderer(canvas,analyser,playing){
    ...
    for(let i=0;i<JET_COUNT;i++)jets.push({t:i/JET_COUNT,angle:ra(0,Math.PI*2),size:ra(0.5,2),bright:ra(0.3,0.8),hue:ri(200,280)})
    ...
    function draw(){
      ...
      // 视界阴影
      const ehSize=SS*0.08
      const sh=ctx.createRadialGradient(CX,CY,0,CX,CY,ehSize*2.5)
      sh.addColorStop(0,"rgba(0,0,0,0.95)")
      sh.addColorStop(0.4,`rgba(0,0,0,${0.8+bass*0.1})`)
      sh.addColorStop(0.7,`rgba(5,0,10,${0.3+bass*0.1})`)
      ...
      for(let i=0;i<100;i++){
        ...
        const dA=(0.03+avgE*0.06+bass*0.04)*pulse*(1-t*0.3)
        ...
        ctx.lineWidth=1+t*3;ctx.stroke()
      }
      ctx.restore()
      ...
      for(const p of particles){
        ...
        const pr=p.radius*SS*0.35
        const px=CX+Math.cos(p.angle+diskRot*(1-p.radius))*pr
        const py=CY+Math.sin(p.angle+diskRot*(1-p.radius))*pr*0.3
        const pA=p.bright*p.life*(0.2+avgE*0.4)*(1-p.radius)+bass*0.15
        if(pA<0.005)continue
        const sz=p.size*(0.3+avgE*0.5)*(1-p.radius*0.5)+0.3
        ctx.beginPath();ctx.arc(px,py,Math.max(0.2,sz),0,Math.PI*2)
        ctx.fillStyle=`hsla(${260-p.radius*60},70%,${50+avgE*20}%,${pA})`
        ctx.fill()
        if(sz>0.8){
          const pg=ctx.createRadialGradient(px,py,0,px,py,sz*3)
          pg.addColorStop(0,`hsla(${270-p.radius*50},60%,55%,${pA*0.2})`);pg.addColorStop(1,"hsla(0,0%,0%,0)")
      ...
      for(const j of jets){
        ...
        if(j.t>1){j.t=0;j.angle=ra(-0.3,0.3);j.size=ra(0.5,2);j.bright=ra(0.3,0.8);j.hue=ri(200,280)}
        const jt=j.t
        const jDist=jt*SS*0.3
        const jSpread=j.dist=jt*SS*0.05
        const jx=CX+Math.sin(j.angle)*jDist
        const jy=CY-Math.cos(j.angle)*jDist
        const ja=j.bright*(1-jt*0.5)*(0.2+avgE*0.3+bass*0.2)
        if(ja<0.003)continue
        const jw=j.size*0.5+jt*j.size*(1+avgE)
        ctx.beginPath();ctx.arc(jx,jy,Math.max(0.3,jw),0,Math.PI*2)
        ctx.fillStyle=`hsla(${j.hue},70%,${60+jt*20}%,${ja})`
        ctx.fill()
        if(jw>1){
          const jg=ctx.createRadialGradient(jx,jy,0,jx,jy,jw*4)
          jg.addColorStop(0,`hsla(${j.hue+10},60%,65%,${ja*0.15})`);jg.addColorStop(1,"hsla(0,0%,0%,0)")
          ctx.fillStyle=jg;ctx.beginPath();ctx.arc(jx,jy,jw*4,0,Math.PI*2);ctx.fill()
        }
      }

      // 视界边缘辉光
      const ehGlow=0.15+bass*0.2+avgE*0.1
      ...
      // 引力透镜畸变
      for(let i=0;i<8;i++){
        const a=(i/8)*Math.PI*2+diskRot*0.5
        const dist=ehSize*(1.6+Math.sin(frame*0.01+i)*0.2)
        const lx=CX+Math.cos(a)*dist,ly=CY+Math.sin(a)*dist
        const lg=ctx.createRadialGradient(lx,ly,0,lx,ly,dist*0.08)
        lg.addColorStop(0,`rgba(255,220,180,${0.02+bass*0.06})`);lg.addColorStop(1,"rgba(0,0,0,0)")
        ctx.fillStyle=lg;ctx.beginPath();ctx.arc(lx,ly,dist*0.08,0,Math.PI*2);ctx.fill()
      }
