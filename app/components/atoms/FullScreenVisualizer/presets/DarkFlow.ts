...
const FLOW_COUNT = 10, PARTICLES_PER_FLOW = 30;
...
function makeFlow(): DFFlow {
  ...
  for (let i = 0; i < 5; i++) {
    pts.push({ x: cx + ra(-0.3, 0.3), y: cy + ra(-0.3, 0.3) });
  }
  const parts: DFFlow["particles"] = [];
  for (let i = 0; i < PARTICLES_PER_FLOW; i++) parts.push({ t: i / PARTICLES_PER_FLOW, size: ra(0.3, 2), bright: ra(0.2, 0.8) });
  return {
    points: pts, speed: ra(0.002, 0.006), width: ra(0.02, 0.08),
    hue: ri(200, 280), alpha: ra(0.06, 0.18), phase: ra(0, Math.PI * 2),
    particles: parts,
  };
}
  ...
  createRenderer(canvas, analyser, playing) {
    ...
    function draw() {
      ...
      for (const f of flows) {
        ...
        ctx.strokeStyle = `hsla(${f.hue},50%,50%,${fAlpha * 0.3})`;
        ctx.lineWidth = f.width * SS * 0.3; ctx.stroke();

        // 外层辉光带
        ctx.strokeStyle = `hsla(${f.hue + 10},40%,60%,${fAlpha * 0.1})`;
        ...
        for (const p of f.particles) {
          ...
          ctx.fillStyle = `hsla(${f.hue + 10},70%,70%,${pAlpha})`;
          ctx.fill();
          if (sz > 1) {
            const pg = ctx.createRadialGradient(px, py, 0, px, py, sz * 4);
            pg.addColorStop(0, `hsla(${f.hue + 15},60%,65%,${pAlpha * 0.15})`);
            pg.addColorStop(1, "hsla(0,0%,0%,0)"); ctx.fillStyle = pg; ctx.beginPath(); ctx.arc(px, py, sz * 4, 0, Math.PI * 2); ctx.fill();
          }
        }
      }
